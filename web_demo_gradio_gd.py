...
parser.add_argument("--model_name", default="OpenMEDLab/gd7bv5", type=str)
parser.add_argument("--instruction_model_name", default="PULSE-COVID-19", type=str)
parser.add_argument("--instruction_model_org", default="OpenMEDLab & Guangzhou Laboratory", type=str)
...
def first_instruction(instruction_model_name, instruction_model_org):
    datetime_p = datetime.datetime.now()
    datetime_s = datetime.datetime.strftime(datetime_p, '%Y-%m-%d')

    return f"Instructions: You are {instruction_model_name}, a large language model trained by {instruction_model_org}. Answer as concisely as possible.\nKnowledge cutoff: 2021-09-01\nCurrent date: {datetime_s}"
...
def predict(
    ...
    input_ids = tokenizer(
        first_instruction(
            args.instruction_model_name,
            args.instruction_model_org,
        ),
...
with gr.Blocks() as demo:
    # 修改 Model Name
    gr.HTML("""<h1 align="center">欢迎使用 PLUSE-COVID-19 </h1>""")
